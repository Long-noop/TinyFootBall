...
// Compile (Linux/mac):
// g++ main.cpp -o tinyfootball.exe -I "D:\TinyFootball\SDL2\project\MinGW\include" -L "D:\TinyFootball\SDL2\project\MinGW\lib" -lmingw32 -lSDL2main -lSDL2 -lSDL2_image -lSDL2_ttf -std=c++17
// On Windows (MinGW):
//-lSDL2_ttf -std=c++17
...
struct Game {
    ...
    bool init(const char* title="Tiny Football (SDL2)"){
        ...
        bgTex = IMG_LoadTexture(renderer, "../kenney_sports-pack/Tilesheet/background.jpg");
        if(!bgTex){
            printf("Warning: Background texture not found, trying alternative...\n");
            bgTex = IMG_LoadTexture(renderer, "../kenney_sports-pack/Tilesheet/background.jpg");
            if(!bgTex){
                printf("Warning: Background texture not found, using default background\n");
                // Tạo texture màu xanh lá cây đơn giản
                bgTex = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, SCREEN_W, SCREEN_H);
                if(bgTex){
        ...
        // Thử load font từ nhiều đường dẫn Windows khác nhau
        font = TTF_OpenFont("C:/Windows/Fonts/arial.ttf", 22);
        if(!font){
            font = TTF_OpenFont("C:/Windows/Fonts/calibri.ttf", 22);
            if(!font){
                font = TTF_OpenFont("./DejaVuSans.ttf", 22);
                if(!font){
                    printf("Warning: could not open font, text rendering may fail\n");
                }
            }
        }
    ...
    void update(float dt){
        ...
        // Giữ nguyên vị trí tâm theo công thức cũ
        ...
        if(ball.x + ball.size >= SCREEN_W + 10){ 
            ball.reset(true);
        }
    ...
    void render(){
        ...
        // draw goals từ elements.png
        // int goalWidth = 30;
        // int goalHeight = SCREEN_H/2;
        // int goalY = SCREEN_H/4;
        // Tỉ lệ thu nhỏ (ví dụ 0.8 = nhỏ 80% so với ban đầu)
        float scale = 0.8f;
        ...
        // Vẫn vẽ vùng goal detection đơn giản (ẩn)
        // SDL_SetRenderDrawColor(renderer, 70,70,70,100);
        // SDL_Rect leftGoalZone = {0, SCREEN_H/3, 6, SCREEN_H/3};
        // SDL_Rect rightGoalZone = {SCREEN_W-6, SCREEN_H/3, 6, SCREEN_H/3};
        // SDL_RenderFillRect(renderer, &leftGoalZone);
        // SDL_RenderFillRect(renderer, &rightGoalZone);
